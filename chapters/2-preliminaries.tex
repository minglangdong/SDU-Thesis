...
\subsection{不经意传输}
\esubsection{Oblivious Transfer}
...
传统的 OT 协议构造往往高度依赖于公钥密码体制，这导致大量执行 OT 时会带来难以承受的计算开销。为了突破这一瓶颈，。Ishai 等人提出了 OT 扩展（OT Extension）技术 \cite{IKNP-CRYPTO-2003}。该技术允许参与方仅通过少量的公钥密码操作生成基础 OT，随后只需执行廉价的对称密码操作便能将其扩展为大量的 OT 实例，从而极大降低了计算开销。近年来，Boyle 等人基于带噪声的奇偶性学习（Learning Parity with Noise, LPN）假设的伪随机相关性生成器（Pseudorandom Correlation Generator, PCG）技术~\cite{BCGIKS19,BCGIKRS19,RRT23} 进一步将 OT 扩展推向了极致，将通信开销压缩至次线性级别。
...
\esubsection{Vector Oblivious Linear Evaluation}
...
\subsection{不经意伪随机函数}\label{sec:boprf}
\esubsection{Oblivious Pseudorandom Function}
...
\subsection{不经意键值存储}\label{sec:okvs}
\esubsection{Oblivious Pseudorandom Function}
...
\esubsection{Oblivious Programmable Pseudorandom Function}
...
\subsection{秘密分享隐私等值测试}\label{sec:sspeqt}
\esubsection{Secret-Shared Private Equality Test}
...
\subsection{多方秘密分享洗牌}\label{sec:ms}
\esubsection{Multi-Party Secret-Shared Shuffle}
...
\subsection{多密钥重随机化公钥加密}\label{sec:mkr-pke}
\esubsection{Multi-Key Rerandomizable Public-Key Encryption}
