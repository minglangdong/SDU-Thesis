...
向量不经意线性求值（Vector Oblivious Linear Evaluation, VOLE）\cite{TODO_VOLE_1} 允许发送方获得接收方持有的两个向量的秘密线性组合。在本文中，我们重点关注 VOLE 的随机变体，为了简便起见，我们在后文中仍将其简称为 VOLE。

令 $\mathbb{F}$ 为某个有限域。在 VOLE 中，参与方没有任何输入。发送方获得一个随机值 $\Delta \in \mathbb{F}$ 以及一个随机向量 $\vec{C} \in \mathbb{F}^m$，而接收方获得两个随机向量 $\vec{A}, \vec{B} \in \mathbb{F}^m$。

我们进一步考虑 VOLE 的一种特例，即允许发送方输入该随机值 $\Delta$。在近期的研究中 \cite{TODO_VOLE_2}，该变体已通过一种被称为伪随机相关性生成器（pseudorandom correlation generator, PCG）的密码学原语得到了高效实现，它允许参与方生成具有相同 $\Delta$ 的多个 VOLE 实例。我们在图 \ref{fig:func-vole} 中给出了该 VOLE 的理想功能 $\mathcal{F}_{\mathsf{vole}}^{\mathbb{F}, m}$。

\begin{figure}[!hbtp]
\begin{framed}
\begin{minipage}[center]{\textwidth}
\begin{trivlist}
\item \textbf{参数：} 两个参与方（发送方 $\Sd$ 和接收方 $\Rcv$）；有限域 $\mathbb{F}$；输出向量的维度 $m$。
\item \textbf{功能：} 
\begin{itemize}[itemsep=2pt,topsep=0pt,parsep=0pt,leftmargin=10pt]
    \item 等待接收来自发送方 $\Sd$ 的输入 $\Delta \in \mathbb{F}$，并等待接收方 $\Rcv$ 参与。
    \item 随机采样 $\vec{A}, \vec{B} \gets \mathbb{F}^m$，并计算 $\vec{C} = \vec{A} \cdot \Delta \oplus \vec{B}$。
    \item 向发送方 $\Sd$ 输出 $\vec{C}$，向接收方 $\Rcv$ 输出 $(\vec{A}, \vec{B})$。
\end{itemize}
\end{trivlist}
\end{minipage}
\end{framed}
\caption{向量不经意线性求值（VOLE）的理想功能 $\mathcal{F}_{\mathsf{vole}}^{\mathbb{F}, m}$}\label{fig:func-vole}
\end{figure}

本文采用子域向量不经意线性求值（subfield vector oblivious linear evaluation, subfield-VOLE）~\cite{BCGIKS19,BCGIKRS19,RRT23} 技术来高效实例化批量不经意伪随机函数。子域 VOLE 的理想功能 $\FuncSubVOLE$ 在图 \ref{fig:func-subvole} 中给出。

\begin{figure}[!hbtp]
\begin{framed}
\begin{minipage}[center]{\textwidth}
\begin{trivlist}
\item \textbf{参数：} 发送方 $\Sd$。接收方 $\Rcv$。输出向量的大小 $n$。定义在基础域 $\mathbb{B}$ 上的扩域 $\mathbb{F}$。
\item \textbf{功能：} 
\begin{itemize}[itemsep=2pt,topsep=0pt,parsep=0pt,leftmargin=10pt]
\item 采样 $\vec{a} \gets \mathbb{B}^n$，$\vec{b} \gets \mathbb{F}^n$，$\Delta \gets \mathbb{F}$。
\item 计算 $\vec{c} = \Delta \vec{a} + \vec{b}$。
\item 将 $(\Delta, \vec{b})$ 发送给 $\Sd$，并将 $(\vec{a}, \vec{c})$ 发送给 $\Rcv$。
\end{itemize}
\end{trivlist}
\end{minipage}
\end{framed}
\caption{子域 VOLE 的理想功能 $\FuncSubVOLE$}\label{fig:func-subvole}
\end{figure}

\subsection{不经意伪随机函数}
\label{sec:boprf}

不经意伪随机函数（Oblivious Pseudorandom Function, OPRF）~\cite{FIPR-TCC-2005} 是隐私集合运算（PSO）领域的核心原语。Kolesnikov 等人~\cite{KKRT-CCS-2016} 引入了批量不经意伪随机函数（Batch OPRF），它通过以下方式提供了一批 OPRF 实例：在第 $i$ 个实例中，接收方 $\Rcv$ 拥有输入 $x_i$；发送方 $\Sd$ 获得一个 PRF 密钥 $k_i$，而 $\Rcv$ 获得伪随机求值结果 $\PRF(k_i, x_i)$。需要注意的是，每个密钥 $\Rcv$ 仅能在一个点上进行 PRF 求值。批量 OPRF 的理想功能 $\FuncbOPRF$ 在图 \ref{fig:func-boprf} 中给出了形式化描述。

\begin{figure}[!hbtp]
\begin{framed}
\begin{minipage}[center]{\textwidth}
\begin{trivlist}
\item \textbf{参数：} 发送方 $\Sd$。接收方 $\Rcv$。批量大小 $B$。集合元素的比特长度 $l$。PRF 族 $\PRF: \{0,1\}^* \times \{0,1\}^* \to \{0,1\}^{\gamma}$。
...
\end{minipage}
\end{framed}
\caption{批量 OPRF 的理想功能 $\FuncbOPRF$}\label{fig:func-boprf}
\end{figure}

% 为了高效实例化批量 OPRF，我们采用了子域向量不经意线性求值（subfield-VOLE）技术~\cite{BCGIKS19,BCGIKRS19,RRT23}。
基于子域 VOLE 的批量 OPRF 协议具体构造如下。为了简便起见，我们假设输入比特长度 $l$ 能够整除 $\lambda$，并令输入 $\vec{x} = (x_1, \cdots , x_n)$ 被视为基础域 $\F{l}$ 中的元素序列。
在离线阶段，发送方 $\Sd$ 和接收方 $\Rcv$ 调用定义在扩域 $\F{\lambda}$（其子域为 $\F{l}$）上的 $\FuncSubVOLE$，其中 $\Sd$ 接收到 $(\Delta, \vec{b})$，$\Rcv$ 接收到 $(\vec{a}, \vec{c})$。根据功能定义，满足等式 $\vec{c} = \Delta \vec{a} + \vec{b}$。
在在线阶段，$\Rcv$ 计算并向 $\Sd$ 发送掩码向量 $\vec{p} = \vec{x} + \vec{a}$。随后，$\Sd$ 定义其批量 OPRF 的密钥为 $(\Delta, \vec{k} = \Delta \vec{p} + \vec{b})$。
底层的 PRF 函数定义为 $\PRF((\Delta, k_i),x_i) = \Hash(i, k_i - \Delta x_i)$，其中 $\Hash: [n] \times \F{\lambda} \to \{0,1\}^\gamma$ 为随机预言机（Random Oracle）。对于第 $i$ 个实例，$\Rcv$ 在本地输出 $\Hash(i, c_i)$。
该构造的正确性可以通过以下等式推导得出：
\begin{gather*}
\begin{aligned}
...
如果一个 KVS 满足不经意性，则称其为不经意键值存储（OKVS）。在我们的协议构造中，还要求 OKVS 满足以下额外的随机性属性：
...
批量 OPPRF 协议可以通过结合上述的批量 OPRF 与 OKVS 高效构造得出~\cite{PSTY-EUROCRYPT-2019,CGS22,RS-EUROCRYPT-2021,RR-CCS-2022}，如图 \ref{fig:proto-bopprf} 所示。
% 这一构造范式构成了所有现有批量 OPPRF 协议 的基础，它们之间的区别仅在于对批量 OPRF 或 OKVS 的具体实例化方式不同。
...
\end{gather*}
$\Sd$ 计算 OKVS 结构 $D = \Encode(A)$ 并将 $D$ 发送给 $\Rcv$。
\item $\Rcv$ 计算向量 $\vec{f}$，其中 $f_i = \Decode(D, x_i) \oplus \PRF(k_i, x_i)$。
\end{enumerate}
\end{trivlist}
\end{minipage}
\end{framed}
\caption{基于 OPRF 和 OKVS 的批量 OPPRF 协议 $\ProtobOPPRF$}\label{fig:proto-bopprf}
\end{figure}
...
本节中，我们将采用分治策略（divide-and-conquer strategy），利用通用 MPC 技术来高效实例化 ssPEQT。该协议的具体构造在图~\ref{fig:proto-sspeqt} 中给出。

\begin{figure}[!hbtp]
\begin{framed}
\begin{minipage}[center]{\textwidth}
\begin{trivlist}
\item \textbf{参数：} 两个参与方 $P_1, P_2$。输入的比特长度为 $\gamma$\footnote{为简便起见，我们假设 $\gamma$ 是 2 的幂。如果不是，可以在双方输入的末尾补 1 对齐。}。
...
\end{minipage}
\end{framed}
\caption{基于通用 MPC 的秘密分享隐私等值测试协议 $\ProtossPEQT$}\label{fig:proto-sspeqt}
\end{figure}
